...
{  
    __IO uint32_t t0 = 0; // 使用32位变量以避免溢出  
  
    // 等待发�?�缓冲区�??  
    while (!(USART1->SR & USART_SR_TXE))  
    {  
        ++t0;  
        if (t0 > 800000) // 增加超时值，确保不会因短暂阻塞�?��??出（根据系统时钟调整�??  
        {  
            return; // 超时返回  
        }  
    }  
    ...
    // 发�?�数�??  
    USART1->DR = data;  //这里的原理是硬件层面的，�?旦写入复制到寄存器，就会进行发�?��??
